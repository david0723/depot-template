# Architecture Generator Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A Node.js script that scans depot-template config files and generates a JSON manifest + Mermaid architecture diagram.

**Architecture:** Single script (`scripts/generate-architecture.js`) with three phases: parse all config files into structured data, build a JSON manifest, render a Mermaid diagram into markdown. One dependency: `js-yaml`.
...
```json
{
  ...
  "private": true,
  "scripts": {
    "architecture": "node scripts/generate-architecture.js"
  },
  "dependencies": {
    "js-yaml": "^4.1.0"
  }
...
Run: `cd /tmp/depot-template && npm install`
...
git commit -m "chore: add package.json with js-yaml dependency"
```

---

### Task 2: Write the Parsing Layer

**Files:**
- Create: `scripts/generate-architecture.js`

This task creates the script file with all parsing functions. The script won't be runnable yet (no main function), but each parser can be tested individually.

- [ ] **Step 1: Create the script with imports and utility function**
...
- [ ] **Step 6: Verify parsers load without errors**

Run: `cd /tmp/depot-template && node -e "require('./scripts/generate-architecture.js')"`
Expected: no output (no errors, no main function yet)

- [ ] **Step 7: Commit**

```bash
git add scripts/generate-architecture.js
git commit -m "feat: add config parsers for architecture generator"
```

---

### Task 3: Write the Build, Render, and Output Layer

**Files:**
- Modify: `scripts/generate-architecture.js`

- [ ] **Step 1: Add buildManifest()**

Append to `scripts/generate-architecture.js`:
...
- [ ] **Step 3: Add writeOutput() and main execution**
...
```bash
git add scripts/generate-architecture.js
git commit -m "feat: add manifest builder, mermaid renderer, and output writer"
```

---

### Task 4: Run, Verify, and Final Commit

**Files:**
- Generated: `docs/architecture.json`
- Generated: `docs/architecture.md`

- [ ] **Step 1: Run the generator**

Run: `cd /tmp/depot-template && npm run architecture`
Expected: `Generated docs/architecture.json and docs/architecture.md`

- [ ] **Step 2: Verify JSON manifest structure**

Run: `cd /tmp/depot-template && node -e "const m = require('./docs/architecture.json'); console.log('agents:', m.agents.length, 'pipelines:', m.pipelines.length, 'skills:', m.skills.length); console.log('worker triggers:', m.agents.find(a=>a.name==='worker')?.triggers.length)"`

Expected: `agents: 3 pipelines: 1 skills: 1` and `worker triggers: 3`

- [ ] **Step 3: Verify Mermaid diagram exists in markdown**

Run: `cd /tmp/depot-template && grep -c 'mermaid' docs/architecture.md && grep -c 'subgraph' docs/architecture.md`

Expected: at least 1 mermaid fence, at least 3 subgraphs (Agents, Pipeline, Skills)

- [ ] **Step 4: Verify all YAML workflows still valid**

Run: `cd /tmp/depot-template && for f in .github/workflows/depot-*.yml; do echo -n "$f: "; python3 -c "import yaml; yaml.safe_load(open('$f')); print('valid')"; done`

Expected: all workflows report `valid` (sanity check that we didn't accidentally modify them)

- [ ] **Step 5: Commit generated output**

```bash
git add docs/architecture.json docs/architecture.md
git commit -m "docs: generate initial factory architecture diagram"
```
